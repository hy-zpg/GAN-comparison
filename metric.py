...
def make_dataset(dataset, dataroot, imageSize,ndc):
    ...
    # three channel
    # elif dataset == 'figr':
    #     dataset = WGAN_data_evaluation(dataroot,imageSize)


    else:
        raise Exception('--dataset must be in cifar10 | lsun | imagenet | folder | lfw | fake | FIGR | Ominiglot')
    assert dataset
    return dataset
...
'''
1. selecting a model
2. extracting feature from the real images or fake images
   fasrures including different requirments:
   such as:
   * pixel: image
   * conv_feture: convolutional feature
   * softmax: classified results
   * logits: classifier, possibility
'''
...
'''
calculing wassertein distance based on eclucian distance 
'''
...
'''
1. calculating the entropy score between x and y
2. all distance is calculated based on eclucian distance
'''
...
'''
1. sampling real images and fake images from training dataset and genreated images.
2. extracting different features for fake images and real images(logits, softmax, con_feature,pixels) from the trained model(popular models)
3. total 31 evaluation metric: 
           * combination of real images and fake images:
             4 feature space(pixel,logits,sofmax,conv_feature) * 7 scores(wasserstein,mmd,knn(5), )
           * another 3 evaluation metric just based on sepcific feature: logits 
